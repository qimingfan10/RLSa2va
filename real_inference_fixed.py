...
# 加载模型（使用官方demo的方式）
print("\n加载模型（使用AutoModelForCausalLM）...")
print("这可能需要几分钟，请耐心等待...")

try:
    from transformers import AutoModelForCausalLM, AutoTokenizer
    
    # 使用AutoModelForCausalLM（官方demo的方式）
    print("  加载模型...")
...
# 选择测试样本
test_samples = annotations[::10][:3]  # 测试3个样本
        ...
        with torch.no_grad():
            ...
            try:
                ...
                result = model.predict_forward(
                    ...
                    text=text,
                    tokenizer=tokenizer,
                    processor=None  # InternVL不需要processor
                )
                
                print("  ✅ 真实推理成功！")
                print(f"  预测文本: {result.get('prediction', 'N/A')}")
                
                # 提取预测掩码
                if 'prediction_masks' in result and result['prediction_masks'] is not None and len(result['prediction_masks']) > 0:
                    pred_masks_list = result['prediction_masks'][0]  # 第一个对象
                    
                    if len(pred_masks_list) > 0:
                        pred_mask = pred_masks_list[0]  # 第一帧
...
# 保存结果
results_json = {
    'model_type': 'HuggingFace (Official Demo Method)',
