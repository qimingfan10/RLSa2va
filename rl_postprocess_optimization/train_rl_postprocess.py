...
from rl_postprocess_optimization.env.postprocess_env import PostProcessOptimizationEnv
...
def make_env(model, tokenizer, dataset, max_steps=5):
    """创建环境"""
    def _init():
        env = PostProcessOptimizationEnv(model, tokenizer, dataset, max_steps)
        env = Monitor(env)
        return env
    return _init
...
def train(args):
    """训练主函数"""
    print("=" * 80)
    print("Sa2VA 后处理优化强化学习训练 - 实验二")
    print("=" * 80)
    
    # 创建输出目录
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_dir = os.path.join(args.output_dir, f"rl_postprocess_{timestamp}")
    ...
    print(f"   - 动作空间: 7个后处理操作")
    
    # 设置回调
    checkpoint_callback = CheckpointCallback(
        save_freq=args.save_freq,
        save_path=os.path.join(output_dir, "checkpoints"),
        name_prefix="ppo_postprocess"
    )
...
def main():
    parser = argparse.ArgumentParser(description="训练Sa2VA 后处理优化RL策略 - 实验二")
    ...
    parser.add_argument(
        '--output_dir',
        type=str,
        default='/home/ubuntu/Sa2VA/rl_postprocess_optimization/outputs',
        help='输出目录'
    )
