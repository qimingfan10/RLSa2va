...
pretrained_pth = "/home/ubuntu/Sa2VA-26B.pth"  # 转换后的pth模型路径

# Data
template = "internlm2_chat"
prompt_template = PROMPT_TEMPLATE.internlm2_chat
max_length = 4096  # 减少序列长度以节省显存

# Scheduler & Optimizer - 针对血管分割任务优化的参数
batch_size = 1  # 每个GPU的批次大小
accumulative_counts = 8  # 梯度累积步数（4卡 x 1 batch x 8 accumulation = 有效batch 32）
dataloader_num_workers = 4  # dataloader workers
max_epochs = 1  # 减少epoch避免过拟合
optim_type = AdamW
lr = 2e-5  # 降低学习率以稳定训练
betas = (0.9, 0.999)
weight_decay = 0.05
...
model = dict(
    ...
    frozen_sam2_decoder=False,  # 不冻结SAM2解码器以适应血管分割
    mllm=dict(
        type=InternVLMLLM,
        model_path=path,
        freeze_llm=False,  # 使用LoRA训练LLM
...
VESSEL_ROOT = DATA_ROOT + 'vessel_data/'
...
randomness = dict(seed=42, deterministic=False)  # 设置随机种子以确保可重现性
