...
from projects.sa2va.models import Sa2VAModel, SAM2TrainRunner, DirectResize, InternVLMLLM, Sa2VAChatMLLM
...
#                          PART 1  Settings                           #
#######################################################################
# Model - 使用Sa2VA-26B模型 (已达到Dice 0.82)
path = "/home/ubuntu/Sa2VA/models/sa2va_vessel_hf"
...
accumulative_counts = 16  # 26B模型需要更大的梯度累积
...
tokenizer = dict(
    ...
    pretrained_model_name_or_path='/home/ubuntu/Sa2VA/models/sa2va_vessel_hf',  # 使用26B模型自带的tokenizer
...
model = dict(
    ...
    mllm=dict(
        type=Sa2VAChatMLLM,  # 使用Sa2VAChatModel专用wrapper
        model_path=path,
        freeze_llm=False,
        freeze_visual_encoder=True,
        llm_lora=dict(
            type=LoraConfig,
            r=32,  # 26B模型使用更小的LoRA rank节省内存
            lora_alpha=64,
            lora_dropout=0.1,
            bias='none',
            task_type='CAUSAL_LM',
