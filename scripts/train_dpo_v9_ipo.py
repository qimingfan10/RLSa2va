#!/usr/bin/env python3
"""
Sa2VA 26B IPO V9 - 使用IPO Loss替代DPO Loss

关键改进：
1. 使用IPO (Identity Preference Optimization) Loss替代DPO Loss
2. IPO是回归Loss，强迫Score(GT)比Score(Pred)高出固定距离
3. 更激进的数据过滤（只保留Baseline错得离谱的样本）
"""
...
def ipo_loss(log_prob_chosen, log_prob_rejected, beta=0.1):
    """
    IPO (Identity Preference Optimization) Loss
    - 不使用sigmoid，直接用平方差
    - 目标是让gap至少达到 1/(2*beta)
    """
    logits = log_prob_chosen - log_prob_rejected
    # IPO: (logits - 1/(2*beta))^2
    target_gap = 1.0 / (2.0 * beta)
    return (logits - target_gap) ** 2
...
class FullForwardDPOTrainer:
    ...
    def __init__(self):
        ...
        self.output_dir = "/home/ubuntu/Sa2VA/work_dirs/sa2va_26b_ipo_v9b"
        self.data_root = "/home/ubuntu/Sa2VA/data/dpo_vessel"
        
        self.beta = 0.05  # 降低beta，允许模型大幅修改权重
        self.dice_weight = 1.0
        self.lr = 1e-6  # 稳定学习率
        self.iou_threshold = 0.7  # 只保留Baseline IoU < 0.7的样本
        self.lora_r = 16
        ...
        print("=" * 60)
        print("🚀 IPO V9 - Identity Preference Optimization")
        print("=" * 60)
    ...
    def _load_and_filter_data(self):
        ...
        for item in annotations:
            ...
            union = (chosen | rejected).sum()
            rejected_iou = inter / (union + 1e-8)
            
            # 只保留Baseline错得离谱的样本（IoU < threshold）
            if rejected_iou < self.iou_threshold:
    ...
    def train_step(self, sample):
        ...
        try:
            ...
            # 使用IPO Loss替代DPO Loss
            loss_ipo = ipo_loss(log_prob_chosen, log_prob_rejected, beta=self.beta)
            
            total_loss = loss_ipo + self.dice_weight * loss_dice
            
            prefer = (dice_with_gt > dice_with_rejected).float().item()
            
            return {
                'loss': total_loss,
                'loss_ipo': loss_ipo.item(),
    ...
    def train(self):
        print("\n🚀 Starting IPO V9 Training...")
        self.model.train()
        
        metrics = {'loss': 0, 'loss_ipo': 0, 'loss_dice': 0, 'dice': 0, 'prefer': 0, 'count': 0}
        global_step = 0
        
        pbar = tqdm(self.filtered_data, desc="IPO V9")
        ...
        for idx, sample in enumerate(pbar):
            ...
            for k in ['loss', 'loss_ipo', 'loss_dice', 'dice', 'prefer']:
        ...
        self._save('final')
        print("\n" + "=" * 60)
        print("🎉 IPO V9 Training completed!")
