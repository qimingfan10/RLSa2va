#!/usr/bin/env python3
"""
Sa2VA 26B DPO训练 V5 - 基于原始训练框架

关键：使用与原始Sa2VA.forward()相同的方式获取[SEG] hidden states
参考: projects/sa2va/models/sa2va.py
"""
...
class DPOTrainerV5:
    def __init__(self):
        ...
        self.output_dir = "/home/ubuntu/Sa2VA/work_dirs/sa2va_26b_dpo_v5"
        ...
        print("=" * 60)
        print("🎯 Sa2VA 26B DPO Training V5 - Original Framework Method")
        print("=" * 60)
    ...
    def _prepare_llm_input(self, image):
        """准备LLM输入，模拟原始框架的方式"""
        # 获取设备
        ...
        pixel_values = img.unsqueeze(0).to(dtype=torch.bfloat16, device=llm_device)
        ...
        input_ids = torch.tensor(input_ids).unsqueeze(0).to(llm_device)
        
        return pixel_values, input_ids
    
    def _forward_llm_get_seg_hidden(self, pixel_values, input_ids):
        """
        通过LLM forward获取[SEG]位置的hidden state
        参考: Sa2VA.forward() in projects/sa2va/models/sa2va.py
        """
        llm_device = next(self.model.language_model.parameters()).device
        ...
        vit_embeds = vit_embeds.to(torch.bfloat16).to(llm_device)
        ...
        # 确保所有张量在相同设备上
        input_ids_local = input_ids.to(llm_device)
        img_context_mask = (input_ids_local == self.model.img_context_token_id)
        ...
        if img_context_mask.sum() > 0:
            ...
            img_positions = img_positions.to(llm_device)
        ...
        # 提取[SEG]位置的hidden state (参考Sa2VA.forward)
        hidden_states = outputs.hidden_states[-1]  # 最后一层
        
        # 通过text_hidden_fcs处理
        hidden_states = self.model.text_hidden_fcs(hidden_states)
        
        # 找[SEG]位置 - 确保在与hidden_states相同的设备上
        hs_device = hidden_states.device
        seg_mask = (input_ids_local.to(hs_device) == self.seg_token_id)
        if seg_mask.sum() == 0:
            return None
        
        # 提取[SEG]的embedding
        pred_embedding = hidden_states[seg_mask]  # [num_seg, 256]
        
        return pred_embedding
    
    def _forward_sam_with_grad(self, image, language_embd):
        """使用SAM2预测mask（带梯度）- 使用训练版本的inject方法"""
        ...
        # 确保language_embd格式正确并在正确设备上
        # 格式: [num_seg, 256] -> [num_seg, 1, 256] for inject_language_embd
        language_embd = language_embd.to(sam2_device)
        if language_embd.dim() == 2:
            language_embd = language_embd.unsqueeze(1)  # [num_seg, 1, 256]
        
        # 获取SAM2 embeddings
        with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
            feats = sam2.sam2_model.forward_image(g_pixel_values)
            _, current_vision_feats, _, feat_sizes = sam2.sam2_model._prepare_backbone_features(feats)
        ...
        # 使用inject_language_embd（训练版本，支持梯度）
        high_res_features = [
        ...
        pix_feat = sam_states['current_vision_feats'][-1] + sam2.sam2_model.no_mem_embed
    ...
    def train_step(self, sample):
        ...
        try:
            # Step 1: 准备LLM输入
            pixel_values, input_ids = self._prepare_llm_input(image)
            
            # Step 2: 通过LLM获取[SEG] hidden state
            pred_embedding = self._forward_llm_get_seg_hidden(pixel_values, input_ids)
            
            if pred_embedding is None:
                return None
            
            # Step 3: 通过SAM2预测mask
            pred_logits = self._forward_sam_with_grad(image, pred_embedding)
    ...
    def train(self):
        print("\n🚀 Starting DPO V5 training...")
        self.model.train()
        
        acc_loss, acc_dice, acc_prefer, acc_count = 0, 0, 0, 0
        global_step = 0
        
        pbar = tqdm(self.annotations, desc="DPO V5 Training")
        ...
        self._save('final')
        print("\n" + "=" * 60)
        print("🎉 DPO V5 Training completed!")
