#!/usr/bin/env python3
"""
Sa2VA V14 - 在全部254个Hard样本上训练

与V13的区别：扫描全部1220个样本，找出所有254个hard样本进行训练
"""
...
class AllHardTrainer:
    """在全部Hard样本上训练"""
    
    def __init__(self):
        self.model_path = "/home/ubuntu/Sa2VA/work_dirs/sa2va_26b_dpo_v8/step_100"
        self.output_dir = "/home/ubuntu/Sa2VA/work_dirs/sa2va_26b_hard_v14"
    ...
    def _find_all_hard_samples(self):
        """扫描全部样本，找出所有Hard样本"""
        print("\n📊 Scanning ALL samples to find hard ones...")
        ...
        for ann in tqdm(valid, desc="Scanning all"):
        ...
        print(f"\n   Found {len(self.hard_samples)} hard samples (Dice < {self.hard_threshold})")
    ...
    def _evaluate_samples(self, samples, desc="Eval"):
        """评估样本"""
        dices_before = []
        dices_after = []
    ...
    def train(self):
        ...
        print(f"\n🏋️ Training on ALL {len(self.hard_samples)} hard samples...")
        ...
        # 评估
        print("\n📊 Evaluating...")
        hard_before, hard_after = self._evaluate_samples(self.hard_samples[:50], "Eval hard")
        ...
        print(f"\n   Random 10 samples:     {random_dice:.4f}")
        print(f"   V8 baseline (10 samp): 0.8189")
